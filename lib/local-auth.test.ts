...
describe("local auth helpers", () => {
  it("normalizes auth emails consistently", () => {
    expect(normalizeAuthEmail("  Test.User@Example.COM ")).toBe(
      "test.user@example.com"
    );
  });

  it("derives a stable fallback display name from the email address", () => {
    expect(deriveNameFromEmail("jane.doe-smith@example.com")).toBe(
      "Jane Doe Smith"
    );
  });
  ...
  it("rejects malformed stored password hashes", async () => {
    await expect(verifyPassword("password", "bad-format")).resolves.toBe(
      false
    );
  });
});